...
useEffect(() => {
  ...
  if (storedData) {
    ...
    const formattedStartTime = parsedData[0]?.
    bookingStart
      ? moment(parsedData[0].
        bookingStart, "x").format("HH:mm")
      : "";

    const formattedEndTime = parsedData[0]?.
    bookingEnd
            ...
            <div className="mt-5">
              ...
              <div className="mt-1">
                <input
                  ...
                  className="w-full px-4 py-2 mt-1 bg-gray-50 rounded-md border border-[#4BB1D3]"
                />
                {errors.ServiceDate && <p className="text-red-500 text-sm">{t("ServiceDateRequired")}</p>}
              </div>
            </div>

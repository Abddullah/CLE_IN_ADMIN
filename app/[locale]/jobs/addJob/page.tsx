...
function page() {
  ...
  // const tax = taxData ? JSON.parse(taxData) : 0;

  // const editData = localStorage.getItem("editJob");
  ...
  const fetchRoomSizes = async () => {
    try {
      ...
      console.log(allroomSizes, "allroomSizes");
  ...
  const handleRoomSizeChange = (value: string) => {
    console.log(value, "handleRoomSizeChange");
    // setSelectedHour(value);
    // totalAmount(value, "roomSize");
  };

  const handleNoOfRoomsChange = (value: string) => {
    console.log(value, "handleNoOfRoomsChange");
  };
